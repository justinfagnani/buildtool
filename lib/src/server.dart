...
class Server {
  ...
  Future<bool> start() {
    return _createLogFile().then((_) {
      ...
      return HttpServer.bind("127.0.0.1", 0).then((server) {
        ...
        router.filter(new Glob('/**'), (HttpRequest r) {
          _logger.info("request: ${r.uri}");
    ...
    req.transform(new StringDecoder()).toList()
      .catchError((e) {
        ...
        _builder.build(data['changed'], data['removed'], clean: data['clean'],
          ...
          onError: (e) {
            var stackTrace = getAttachedStackTrace(e);
  ...
  Future<int> _writeLockFile(Path baseDir, int port) {
    ...
    if (lockFile.existsSync()) {
      ...
      return new Future(() => int.parse(contents))
        .then((int otherPort) {
  ...
  Future _closeLogFile() =>
      _logSink == null ? new Future.value(null) : (_logSink..close()).done;
